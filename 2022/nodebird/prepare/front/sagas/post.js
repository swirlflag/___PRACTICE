...
import { ADD_POST_ME, REMOVE_POST_ME } from "../reducers/user";

const API_getPosts = (lastId) => {
	return axios.get(`/api/posts?lastId=${lastId}`);
};
const API_getUserPosts = (userId, lastId) => {
	return axios.get(`/api/user/${userId}/posts?lastId=${lastId}`);
};
const API_getHashtagPosts = (hashtag, lastId ) => {
    hashtag = encodeURIComponent(hashtag);
	return axios.get(`/api/hashtag/${hashtag}?lastId=${lastId}`);
};
const API_getPost = (postId) => {
...
function* loadUserPosts(action) {
	const { userId, lastId } = action.data;
	try {
		const { data: resData } = yield call(API_getUserPosts, userId, lastId);
		yield put({
...
function* loadHashtagPosts(action) {
	const { hashtag, lastId } = action.data;
	try {
		const { data: resData } = yield call(API_getHashtagPosts, hashtag, lastId);
		yield put({
...
function* loadPost(action) {
	...
	try {
		const { data: resData } = yield call(API_getPost, postId);
		yield put({
